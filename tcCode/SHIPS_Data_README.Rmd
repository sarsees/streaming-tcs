...
**TIME** | num. The time interval relative to the start of the storm in 6 hour increments (0 = initial, 1 = 6hrs, 2 = 12hrs ...)
...
load("~/Documents/Projects/TeMpSA/TeMpSA/SHIPS_Developmental/tcStormsv2.RData")
```
Without any further cleaning, the data is ``r nrow(tcStorms)`` rows of realtime instances and ``r ncol(tcStorms)`` columns of attributes to each instance. There are ``r length(unique(tcStorms$ID))`` unique TCs ranging from ``r min(tcStorms$DATE)`` to ``r max(tcStorms$DATE)``. The storm paths are again limited to the Atlantic Basin as seen below.
...
RI_fun <- function(VMAX_Obs, cut=30, numeric = FALSE){
  ...
  if(N>4){
    VMAX_Off4 <- VMAX_Obs[-c(1:4)] #replicate of VMAX_Obs but delayed 24 hours
    
    VMAX_Diff <- VMAX_Off4-VMAX_Obs[-c((N-3):N)] #Index of events that should be classified as RI
    
    if(numeric){
      res[-c((N-3):N)] <- VMAX_Diff
    }else{
      res[which(VMAX_Diff>=cut)] <- 1
    }
  }
...
byStorm <- group_by(tcStorms, ID) %>% mutate(RI = RI_fun(VMAX), VMAX_Diff = RI_fun(VMAX, numeric = TRUE), DELV = (VMAX - first(VMAX)))
...
areTheyIncreasing$TIME <- byStorm$TIME
...
show history inconsistencies, which under further investigation, is a result of date inconsistencies. 

### VMAX and TIME
The storm instances have a binary label, 1 indicating RI and 0 indicating nonRI, in addition to a VMAX_Diff column that measures the future 24 hour change in intensity relative to the current time point. The distribution of maximum intensities per time interval for the nonRI and RI storms are shown below
...
ggplot(byIndex, aes(x = VMAX_Diff, fill = factor(RI)))+
  geom_histogram()+
  scale_fill_manual(values = c("#999999", "#E69F00"), name = "Label", breaks = c(0,1), labels = c("nonRI", "RI"))
```

### Time Component
```{r, echo = FALSE, message = FALSE}
byStormSummary <- byStorm %>% summarise(nObs = n(), RI_Label = sum(RI))
```
The longest storm in the data is ``r byStormSummary[which.max(byStormSummary$nObs), "ID"]`` with a total of ``r max(byStormSummary$nObs)`` instances including the initial ( or ``r (max(byStormSummary$nObs) - 1)*6`` hours). The shortest storm is  ``r byStormSummary[which.min(byStormSummary$nObs), "ID"]`` with a total of ``r min(byStormSummary$nObs)`` instances including the initial ( or ``r (min(byStormSummary$nObs) - 1)*6`` hours). The distribution of storm duration for all storms, colored by the number of RI events in each storm, looks like:

```{r, echo = FALSE, message = FALSE}

p <- wes_palette("Royal1", n = 12, type = "continuous")
ggplot(byStormSummary, aes(x = nObs, fill = factor(RI_Label)))+
  geom_histogram()+
  ggtitle("Storm Lengths and RI Instances")+
  xlab("Number of 6-Hour-Interval Observations")+
...
a1 <- ggplot(nonRIFour, aes(x = TIME, y = VMAX, color = ID)) +
  geom_line() +
...
a2 <- ggplot(RIFour, aes(x = TIME, y = VMAX, color = ID)) +
  geom_line() +
  coord_cartesian(ylim = c(0, 170)) + 
...
write.csv(byStorm, file = "~/Documents/Projects/TeMpSA/TeMpSA/SHIPS_Developmental/tcStorms.csv", row.names = FALSE)
```
# References
