...
tcPost83set <- byStorm %>% filter(!(ID %in% dissStormIDs)) %>% filter(TIMESTAMP > parse_date_time(1983, "y")) %>% select(-c(RecordNum, RI, VMAX_Diff, Match))
```
...
### By Year
#### Up to 1995 (excluding all satellites)
```{r, echo = FALSE}
pre95Feats <- byStorm[,!(grepl( "IR." , names(byStorm)))] # remove IR data
byYearTo94 <-  pre95Feats %>% 
  filter(!(ID %in% dissStormIDs)) %>% 
  select(-c(RI,VMAX_Diff, RecordNum, RD20, RD26, RHCN, Match)) %>% 
  mutate(YEAR = year(DATE)) %>% 
  group_by(YEAR) %>% 
  filter(YEAR < 1995)
yearDataTo94 <- data.frame(byYearTo94)
yearDataTo94$Missing <- apply(yearDataTo94, 1, function(x) any(is.na(x)))
missingSummarybyYearTo94 <- yearDataTo94 %>% group_by(YEAR) %>% summarise(TimePointsWithNAs = sum(Missing), TotalTimePoints = n())
missingSummarybyYearTo94
```

#### 1995 to 2014 Inclusive (excluding RD20, RD26 satellite)

```{r, echo = FALSE}
post95Feats <- byStorm[,!(grepl( "IRM3." , names(byStorm)))] 
byYear <-  post95Feats %>% 
  filter(!(ID %in% dissStormIDs)) %>% 
  select(-c(RI,VMAX_Diff, RecordNum, Match, RD20, RD26, RHCN, INCV)) %>% 
  mutate(YEAR = year(DATE)) %>% 
  group_by(YEAR) %>% 
  filter(YEAR >= 1995)
yearDataPast94 <- data.frame(byYear)
yearDataPast94$Missing <- apply(yearDataPast94, 1, function(x) any(is.na(x)))
missingSummarybyYearPast94 <- yearDataPast94 %>% group_by(YEAR) %>% summarise(TimePointsWithNAs = sum(Missing), TotalTimePoints = n())
missingSummarybyYearPast94 
```
