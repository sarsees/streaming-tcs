...
**TIMESTAMP** | POSIXct. Date of the storm in the format *yymmdd HH:MM:SS* when available
...
byStorm <- group_by(tcData, ID) %>% mutate(RI = RI_fun(VMAX), VMAX_Diff = RI_fun(VMAX, numeric = TRUE), DELV = (VMAX - first(VMAX)))
...
byStorm$LAST <- NULL
# Fix TIMESTAMP
byStorm$TIMESTAMP <- parse_date_time(byStorm$TIMESTAMP, "%y%m%d %H%M%S")
