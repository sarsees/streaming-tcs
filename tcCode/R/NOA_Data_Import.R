...
MatchTimePoints <- function(datDir = "~/Desktop/TC_Data/New/", datPattern = ".csv"){ 
  ...
  for (i in 2:length(groupedPaths$paths)) {
    ...
    # Get the GOES Variables For Current Time
    tcStorms[i, "IR00_TIME"] <- currentRecord[which(currentRecord$TIME == -12), "IR00"]
    tcStorms[i, "IR00_AVG_200BT"] <- currentRecord[which(currentRecord$TIME == -6), "IR00"]
    tcStorms[i, "IR00_STD_200BT"] <- currentRecord[which(currentRecord$TIME == 0), "IR00"]
    tcStorms[i, "IR00_AVG_300BT"] <- currentRecord[which(currentRecord$TIME == 6), "IR00"]
    tcStorms[i, "IR00_STD_300BT"] <- currentRecord[which(currentRecord$TIME == 12), "IR00"]
    tcStorms[i, "IR00_PCT_AREA_10BT"] <- currentRecord[which(currentRecord$TIME == 18), "IR00"]
    tcStorms[i, "IR00_PCT_AREA_20BT"] <- currentRecord[which(currentRecord$TIME == 24), "IR00"]
    tcStorms[i, "IR00_PCT_AREA_30BT"] <- currentRecord[which(currentRecord$TIME == 30), "IR00"]
    tcStorms[i, "IR00_PCT_AREA_40BT"] <- currentRecord[which(currentRecord$TIME == 36), "IR00"]
    tcStorms[i, "IR00_PCT_AREA_50BT"] <- currentRecord[which(currentRecord$TIME == 42), "IR00"]
    tcStorms[i, "IR00_PCT_AREA_60BT"] <- currentRecord[which(currentRecord$TIME == 48), "IR00"]
    tcStorms[i, "IR00_MAX_BT"] <- currentRecord[which(currentRecord$TIME == 54), "IR00"]
    tcStorms[i, "IR00_AVG_30BT"] <- currentRecord[which(currentRecord$TIME == 60), "IR00"]
    tcStorms[i, "IR00_RADIUS_MAXBT"] <- currentRecord[which(currentRecord$TIME == 66), "IR00"]
    tcStorms[i, "IR00_MIN_20BT"] <- currentRecord[which(currentRecord$TIME == 72), "IR00"]
    tcStorms[i, "IR00_AVG_20BT"] <- currentRecord[which(currentRecord$TIME == 78), "IR00"]
    tcStorms[i, "IR00_RADIUS_MINBT"] <- currentRecord[which(currentRecord$TIME == 78), "IR00"]
    # Get the GOES Variables For 3 Hrs  Time
    tcStorms[i, "IRM3_TIME"] <- currentRecord[which(currentRecord$TIME == -12), "IRM3"]
    tcStorms[i, "IRM3_AVG_200BT"] <- currentRecord[which(currentRecord$TIME == -6), "IRM3"]
    tcStorms[i, "IRM3_STD_200BT"] <- currentRecord[which(currentRecord$TIME == 0), "IRM3"]
    tcStorms[i, "IRM3_AVG_300BT"] <- currentRecord[which(currentRecord$TIME == 6), "IRM3"]
    tcStorms[i, "IRM3_STD_300BT"] <- currentRecord[which(currentRecord$TIME == 12), "IRM3"]
    tcStorms[i, "IRM3_PCT_AREA_10BT"] <- currentRecord[which(currentRecord$TIME == 18), "IRM3"]
    tcStorms[i, "IRM3_PCT_AREA_20BT"] <- currentRecord[which(currentRecord$TIME == 24), "IRM3"]
    tcStorms[i, "IRM3_PCT_AREA_30BT"] <- currentRecord[which(currentRecord$TIME == 30), "IRM3"]
    tcStorms[i, "IRM3_PCT_AREA_40BT"] <- currentRecord[which(currentRecord$TIME == 36), "IRM3"]
    tcStorms[i, "IRM3_PCT_AREA_50BT"] <- currentRecord[which(currentRecord$TIME == 42), "IRM3"]
    tcStorms[i, "IRM3_PCT_AREA_60BT"] <- currentRecord[which(currentRecord$TIME == 48), "IRM3"]
    tcStorms[i, "IRM3_MAX_BT"] <- currentRecord[which(currentRecord$TIME == 54), "IRM3"]
    tcStorms[i, "IRM3_AVG_30BT"] <- currentRecord[which(currentRecord$TIME == 60), "IRM3"]
    tcStorms[i, "IRM3_RADIUS_MAXBT"] <- currentRecord[which(currentRecord$TIME == 66), "IRM3"]
    tcStorms[i, "IRM3_MIN_20BT"] <- currentRecord[which(currentRecord$TIME == 72), "IRM3"]
    tcStorms[i, "IRM3_AVG_20BT"] <- currentRecord[which(currentRecord$TIME == 78), "IRM3"]
    tcStorms[i, "IRM3_RADIUS_MINBT"] <- currentRecord[which(currentRecord$TIME == 78), "IRM3"]
  }
